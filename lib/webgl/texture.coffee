...
exports.Texture2D = class Texture2D extends Texture
    constructor: (@gl, {@channels, @format}={}) ->
        super()
        @channels ?= @gl.RGBA
        @format ?= @gl.RGBA
    ...
    uploadImage: (image) ->
        @width = image.width
        @height = image.height
        @gl.texImage2D @target, 0, @channels, @format, @gl.UNSIGNED_BYTE, image
        return @

    uploadData: (data, @width, @height, type=@gl.UNSIGNED_BYTE) ->
        @gl.texImage2D @target, 0, @channels, width, height, 0, @format, type, data
        return @

    setSize: (@width, @height, type=@gl.UNSIGNED_BYTE) ->
